...
partir dos hosts e dos roteadores para validar a sua configuração.
   ...
   ```plaintext
   Perceba que R5 não sabe que a rede 200.151.10.0 possui subredes (usa uma máscara /24),
   ainda sim, o roteamento funcionará pois a distribuição de pacotes entre as subredes
   é feita por R1 e R2. 

   Portanto, não é necessário, para nenhum dos nodos da rede 200.251.10.0 saberem a 
   máscara correta de 200.151.10.0
   ```
